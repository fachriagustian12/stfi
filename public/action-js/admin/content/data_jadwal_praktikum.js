      ...
      if (code != "0") {
        var dt = $("#all_jadwal_praktikum").DataTable({
          ...
          aoColumns: [
            ...
            { mDataProp: "nm_dosen", class: "text-center", width: "10%" },
  ...
  if (mode == "delete") {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        $("#mata_kuliah_praktikum").val(data.mata_kuliah_praktikum);
        $("#nama_kelompok").val(data.nama_kelompok);
        $("#jam_mulai").val(data.jam_mulai);
        $("#jam_akhir").val(data.jam_akhir);
        $("#nama_hari").val(data.nama_hari);
        $("#nm_dosen").val(data.kd_dosen);
        $("wrd").html("Update");
      },
    });
  }
